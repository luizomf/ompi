...
generation. All three intents use the fixed `gpt-6-astra` model with medium
reasoning; callers cannot select model or effort. The extension passes explicit
model and effort flags, independent of the helper's own profile defaults.
Exact-URL retrieval retains the helper's `quick` profile, while research and
image retain `research`.
