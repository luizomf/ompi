...
describe("tmux status extension", () => {
  ...
  it("publishes the Pi session name and running lifecycle to its tmux window", async () => {
    ...
    expect(invocations).toEqual([
      {
        command: "tmux",
        args: ["set-option", "-w", "-t", "%7", "@pi_status", "󰏤 IDLE leak scan"],
      },
      {
        command: "tmux",
        args: ["set-option", "-w", "-t", "%7", "@pi_status", "󰐊 RUN  leak scan"],
      },
      {
        command: "tmux",
        args: ["set-option", "-w", "-t", "%7", "@pi_status", "󰏤 IDLE leak scan"],
  ...
  it("uses the project directory until the Pi session receives a name", async () => {
    ...
    expect(statuses).toEqual(["󰏤 IDLE dotfiles", "󰏤 IDLE architecture"]);
  });
  ...
  it("keeps the published status on one compact line", async () => {
    ...
    tmuxStatusExtension(pi);
    await handlers.get("session_start")?.({}, { cwd: "/work/dotfiles" } as ExtensionContext);

    expect(status).toBe("󰏤 IDLE review leaked credentials");
  });
